...
MSK(wsc2026-sensor-raw) 에서 원시 센서 데이터를 배치 수신하여
  - NORMAL : DynamoDB(wsc2026-sensor-data) 저장
  - ALERT  : alert_reason 추가 후 wsc2026-sensor-alert 토픽으로 전송
"""
...
# -----------------------------------------------------------------------------
# 이상 탐지
#   temperature > 80 / < 10 , humidity > 90 / < 20  -> ALERT
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# alert 토픽 producer (kafka 라이브러리가 있을 때만 동작)
# -----------------------------------------------------------------------------
def get_producer():
    global _producer
